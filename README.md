...
code .
# VS Codeが自動的に"Reopen in Container"を提案

# 3. 自動セットアップ完了まで待機
# postCreateCommand で make setup-dev が自動実行
# Go、Python、ML環境が全て自動準備
```

#### 🍎 ARM64 Mac（M1/M2/M3）環境
```bash
# 1. リポジトリクローン
git clone https://github.com/nyasuto/bee
cd bee

# 2. ARM64専用DevContainer設定
cp .devcontainer/devcontainer-arm64.json .devcontainer/devcontainer.json
code .

# 3. ARM64最適化セットアップ
# make setup-arm64 が自動実行
# TensorFlow比較はホストMac推奨
```

**DevContainerのメリット：**
- ✅ **環境統一**: 全開発者で完全に同一の開発環境
- ✅ **Claude Code最適化**: AI Agent駆動開発に最適化された環境  
- ✅ **自動依存関係**: Go + Python + GPU環境の自動セットアップ
- ✅ **即座開始**: 複雑な設定不要で10分で開発開始
- 🍎 **ARM64対応**: Apple Silicon Mac最適化済み
